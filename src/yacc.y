...
postfix_expression
	...
	| postfix_expression PTR_OP IDENTIFIER					{$$=nonterminal3("postfix_expression",$1,terminal($2),terminal($3));}
	| postfix_expression INC_OP							   {$$=nonterminal2("postfix_expression", $1, terminal($2));}
	| postfix_expression DEC_OP								 {$$=nonterminal2("postfix_expression", $1, terminal($2));}
	;
...
unary_expression
	: postfix_expression									{$$=$1;}
	| INC_OP unary_expression								{$$=nonterminal2("unary_expression",terminal($1),$2);}
	| DEC_OP unary_expression								{$$=nonterminal2("unary_expression",terminal($1),$2);}
	| unary_operator cast_expression						{$$=nonterminal2("unary_expression",$1,$2);}
	| SIZEOF unary_expression								{$$=nonterminal2("unary_expression",terminal($1),$2);}
	| SIZEOF '(' type_name ')'								{$$=nonterminal2("unary_expression",terminal($1),$3);}
	;
...
multiplicative_expression
	: cast_expression									   {$$=$1;}
	| multiplicative_expression '*' cast_expression        {$$=nonterminal2("*", $1, $3);}
	| multiplicative_expression '/' cast_expression        {$$=nonterminal2("/", $1, $3);}
	| multiplicative_expression '%' cast_expression        {$$=nonterminal2("%", $1, $3);}
	;
...
additive_expression
	...
	| additive_expression '+' multiplicative_expression     {$$=nonterminal2("+", $1, $3);}
	| additive_expression '-' multiplicative_expression     {$$=nonterminal2("-", $1, $3);}
	;

shift_expression
	: additive_expression									{$$=$1;}
	| shift_expression LEFT_OP additive_expression		{$$=nonterminal2("<<",$1,$3);}
	| shift_expression RIGHT_OP additive_expression     {$$=nonterminal2(">>",$1,$3);}
	;

relational_expression	
	: shift_expression										{$$=$1;}
	| relational_expression '<' shift_expression   {$$=nonterminal2(">",$1,$3);}
	| relational_expression '>' shift_expression   {$$=nonterminal2("<",$1,$3);}
	| relational_expression LE_OP shift_expression {$$=nonterminal2("<=",$1,$3);}
	| relational_expression GE_OP shift_expression {$$=nonterminal2(">=",$1,$3);}
	;

equality_expression
	: relational_expression									{$$=$1;}
	| equality_expression EQ_OP relational_expression       {$$=nonterminal2("==",$1,$3);}
	| equality_expression NE_OP relational_expression       {$$=nonterminal2("!=",$1,$3);}
	;

and_expression
	: equality_expression									       {$$=$1;}
	| and_expression '&' equality_expression                       {$$=nonterminal2("&",$1,$3);}
	;

exclusive_or_expression
	: and_expression											    {$$=$1;}
	| exclusive_or_expression '^' and_expression			{$$=nonterminal2("^",$1,$3);}
	;

inclusive_or_expression
	: exclusive_or_expression										{$$=$1;}
	| inclusive_or_expression '|' exclusive_or_expression			{$$=nonterminal2("|",$1,$3);}
	;

logical_and_expression
	: inclusive_or_expression										{$$=$1;}
	| logical_and_expression AND_OP inclusive_or_expression			{$$=nonterminal2("&&",$1,$3);}
	;

logical_or_expression
	: logical_and_expression										{$$=$1;}
	| logical_or_expression OR_OP logical_and_expression			{$$=nonterminal2("||",$1,$3);}
	;

/*check & vs && */
conditional_expression
	: logical_or_expression												{$$=$1;}
	| logical_or_expression '?' expression ':' conditional_expression    {$$=nonterminal3("conditional_expression",$1,$3,$5);}
	;
...
assignment_operator
	...
	| ADD_ASSIGN		{$$=terminal("+=");}
	| SUB_ASSIGN		{$$=terminal("-=");}
	| LEFT_ASSIGN		{$$=terminal("<<=");}
	| RIGHT_ASSIGN		{$$=terminal(">>=");}
	| AND_ASSIGN		{$$=terminal("&=");}
	| XOR_ASSIGN		{$$=terminal("^=");}
	| OR_ASSIGN		   {$$=terminal("|=");}
	;
...
enum_specifier
	: ENUM '{' enumerator_list '}'					{$$=nonterminal2("enum_specifier",terminal($1),$3);}
	| ENUM IDENTIFIER '{' enumerator_list '}'	   {$$=nonterminal3("enum_specifier",terminal($1),terminal($2),$4);}
	| ENUM IDENTIFIER								{$$=nonterminal2("enum_specifier",terminal($1),terminal($2));}
	;
...
enumerator
	: IDENTIFIER										{$$=terminal($1);}
	| IDENTIFIER '=' constant_expression                {$$=nonterminal2("=",terminal($1),$3);}
	;
...
direct_declarator
	...
	| direct_declarator '[' ']'							  {$$=nonterminal2("direct_declarator",$1,terminal("[]"));}
...
parameter_type_list
	: parameter_list											{$$=$1;}
	| parameter_list ',' ELLIPSIS								{$$=nonterminal2("parameter_type_list",$1,terminal($3));}
	;
...
direct_abstract_declarator
	...
	| '[' ']'							{$$=terminal("[ ]");}
	| '[' constant_expression ']'		{$$=$2;}
	| direct_abstract_declarator '[' ']'			{$$=nonterminal2("direct_abstract_declarator",$1,terminal("[]"));}
	| direct_abstract_declarator '[' constant_expression ']'  {$$ = nonterminal2("direct_abstract_declarator", $1, $3);;}
	| '(' ')'							{$$ = terminal("( )");}
	| '(' parameter_type_list ')'		{$$=$2;}
	| direct_abstract_declarator '(' ')'				{$$=nonterminal2("direct_abstract_declarator",$1,terminal("()"));}
	| direct_abstract_declarator '(' parameter_type_list ')'    {$$=nonterminal2("direct_abstract_declarator",$1,$3);}
	;
...
initializer_list
	: initializer											{$$=$1;}
	| initializer_list ',' initializer						{$$=nonterminal2("initializer_list",$1,$3);}
	;
